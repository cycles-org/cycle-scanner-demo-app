...
> - **(c)** — **Partially fixed.** Modal MATCHING is fixed: typing "Apple" now finds `AAPL`/`0R2V`/`603020` natively because the modal matches against both `instrument.symbol` AND `instrument.company`. **BUT** the row template also changed: each row is `_Left` (symbol only) + `_Right` (exchange + type) and the `company` field moved to the row's `title` attribute (hover tooltip only) — it's **no longer rendered inline**. Result: three Apple-related rows all read "AAPL US STOCK" with no way to disambiguate at a glance. Our symbol-augmentation workaround stays — its role shifted from "force the substring match" (pre-3.1.7) to "put the company name back where users can see it without hovering" (post-3.1.7). Cosmetic: `Instrument.filter`'s first parameter renamed `symbol` → `query` in docs.
